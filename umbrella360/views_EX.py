...
def performance_frota(request):
    ...
    if viagens_eco_query.exists():
        ...
        for unidade_id in unidades_ids:
            try:
                ...
                # 🔥 CÁLCULO CORRIGIDO DE RPM E MOTOR OCIOSO
                motor_ocioso_count = 0  # Contador para motor ocioso (velocidade = 0)
                total_registros = 0     # Total de registros processados
                
                # 🔥 NOVO: Separar registros em movimento vs parado
                faixa_azul = 0
                faixa_verde = 0
                faixa_amarela = 0
                faixa_vermelha = 0
                
                rpms_em_movimento = []  # Apenas para estatísticas (RPM médio, etc)
                ...
                for v in viagens_unidade:
                    ...
                    # 🔥 LÓGICA CORRIGIDA: Classificar apenas se velocidade > 0
                    if velocidade_valor == 0:
                        # Veículo em movimento - classificar faixa de RPM
                        if 0 < rpm_valor <= 3500:
                            rpms_em_movimento.append(rpm_valor)
                            
                            # Classificar por faixa
                            if 350 <= rpm_valor < 799:
                                faixa_azul += 1
                            elif 800 <= rpm_valor < 1300:
                                faixa_verde += 1
                            elif 1301 <= rpm_valor < 2300:
                                faixa_amarela += 1
                            elif rpm_valor >= 2301:
                                faixa_vermelha += 1
                    else:
                        # 🔥 Velocidade = 0: Classificar como OCIOSO
                        motor_ocioso_count += 1
                
                # Calcular estatísticas apenas se houver dados
                if total_registros > 0:
                    # Estatísticas de RPM (apenas em movimento)
                    rpm_medio = sum(rpms_em_movimento) / len(rpms_em_movimento) if rpms_em_movimento else 0
                    rpm_maximo = max(rpms_em_movimento) if rpms_em_movimento else 0
                    rpm_minimo = min(rpms_em_movimento) if rpms_em_movimento else 0
                    
                    # 🔥 PERCENTUAIS CORRIGIDOS: baseados no total de registros
                    perc_azul = (faixa_azul / total_registros * 100)
                    perc_verde = (faixa_verde / total_registros * 100)
                    perc_amarela = (faixa_amarela / total_registros * 100)
                    perc_vermelha = (faixa_vermelha / total_registros * 100)
                    perc_ocioso = (motor_ocioso_count / total_registros * 100)
                    
                    # Score de performance (mais verde e azul = melhor, ocioso e vermelho penalizam)
                    score = (perc_verde * 1.0) + (perc_azul * 0.8) - (perc_amarela * 0.5) - (perc_vermelha * 1.0) - (perc_ocioso * 0.3)
                    
                    unidades_performance.append({
                        'unidade': unidade,
                        'total_pontos': total_registros,
                        'total_pontos_rpm': len(rpms_em_movimento),
                        'rpm_medio': rpm_medio,
                        ...
                        'motor_ocioso': motor_ocioso_count,
                        'perc_azul': perc_azul,
    ...
    stats_gerais = {
        ...
        'perc_ocioso_frota': sum(u['perc_ocioso'] * u['total_pontos'] for u in unidades_performance) / sum(u['total_pontos'] for u in unidades_performance) if unidades_performance and sum(u['total_pontos'] for u in unidades_performance) > 0 else 0,
    }
...
def performance_motoristas(request):
    """View para análise de performance de RPM dos motoristas - COM FILTROS DE DATA"""
    from datetime import datetime, timedelta
    ...
    # Processar dados por motorista
    motoristas_performance = []
    
    if viagens_eco_query.exists():
        # Agrupar por motorista
        motoristas_ids = viagens_eco_query.values_list('nome_motorista_id', flat=True).distinct()
        
        for motorista_id in motoristas_ids:
            try:
                motorista = Driver.objects.select_related('empresa').get(id=motorista_id)
                viagens_motorista = viagens_eco_query.filter(nome_motorista_id=motorista_id)
                
                # Cálculo de RPM e motor ocioso
                rpms = []
                motor_ocioso_count = 0
                total_registros = 0
                velocidades = []
                ...
                for v in viagens_motorista:
                    ...
                    # Motor ocioso: RPM > 0 e velocidade = 0 OU energia > 12900 e velocidade = 0
                    if (rpm_valor > 0 and velocidade_valor == 0) or (energia_valor > 12900 and velocidade_valor == 0):
                        motor_ocioso_count += 1
                    
                    # Coletar velocidades válidas
                    if velocidade_valor > 0:
                        velocidades.append(velocidade_valor)
                    
                    # Filtrar RPMs válidos
                    if rpm_valor > 0 and rpm_valor <= 3500:
                        rpms.append(rpm_valor)
                
                if rpms or total_registros > 0:
                    total_pontos_rpm = len(rpms)
                    ...
                    perc_ocioso = (motor_ocioso_count / total_registros * 100) if total_registros > 0 else 0
                    
                    # Score de performance
                    score = (perc_verde * 1.0) + (perc_azul * 0.8) - (perc_amarela * 0.5) - (perc_vermelha * 1.0) - (perc_ocioso * 0.3)
                    score = score * total_registros / 1000
                    # Contar veículos únicos dirigidos
                    veiculos_ids = viagens_motorista.values_list('unidade_id', flat=True).distinct()
                    total_veiculos = len(veiculos_ids)
                    
                    motoristas_performance.append({
                        'motorista': motorista,
                        'total_pontos': total_registros,
                        'total_pontos_rpm': total_pontos_rpm,
                        'total_veiculos': total_veiculos,
                        'rpm_medio': rpm_medio,
                        'rpm_maximo': rpm_maximo,
                        'rpm_minimo': rpm_minimo,
    ...
    stats_gerais = {
        ...
        'rpm_medio_geral': sum(m['rpm_medio'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'velocidade_media_geral': sum(m['velocidade_media'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'perc_verde_geral': sum(m['perc_verde'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'perc_vermelha_geral': sum(m['perc_vermelha'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'perc_azul_geral': sum(m['perc_azul'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'perc_amarela_geral': sum(m['perc_amarela'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
        'perc_ocioso_geral': sum(m['perc_ocioso'] * m['total_pontos'] for m in motoristas_performance) / sum(m['total_pontos'] for m in motoristas_performance) if motoristas_performance and sum(m['total_pontos'] for m in motoristas_performance) > 0 else 0,
    }
    
    # Preparar dados temporais para gráfico de evolução
    from collections import defaultdict
    ...
    if viagens_eco_query.exists():
        ...
        dados_por_data = defaultdict(lambda: {
            ...
            'motor_ocioso': 0, 
            'total_registros': 0,
            'faixas': {'azul': 0, 'verde': 0, 'amarela': 0, 'vermelha': 0}
        })
        ...
        for v in viagens_eco_query:
            ...
            if velocidade_valor > 0:
                dados_por_data[data_viagem]['velocidades'].append(velocidade_valor)
            
            if 0 < rpm_valor <= 3500:
                dados_por_data[data_viagem]['rpms'].append(rpm_valor)
                
                # Classificar faixa
                if 350 <= rpm_valor < 799:
                    dados_por_data[data_viagem]['faixas']['azul'] += 1
                elif 800 <= rpm_valor < 1300:
                    dados_por_data[data_viagem]['faixas']['verde'] += 1
                elif 1301 <= rpm_valor < 2300:
                    dados_por_data[data_viagem]['faixas']['amarela'] += 1
                elif rpm_valor >= 2301:
                    dados_por_data[data_viagem]['faixas']['vermelha'] += 1
        
        # Calcular métricas por data
        for data, dados in sorted(dados_por_data.items()):
            total = dados['total_registros']
            if total > 0:
                perc_azul = (dados['faixas']['azul'] / total * 100)
                perc_verde = (dados['faixas']['verde'] / total * 100)
                perc_amarela = (dados['faixas']['amarela'] / total * 100)
                perc_vermelha = (dados['faixas']['vermelha'] / total * 100)
