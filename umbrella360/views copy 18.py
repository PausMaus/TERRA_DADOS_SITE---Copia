...
def jornada_motoristas(request):
    ...
    if viagens_eco_query.exists():
        ...
        for motorista_id in motoristas_ids:
            try:
                motorista = Driver.objects.select_related('empresa').get(id=motorista_id)
                viagens_motorista = viagens_eco_query.filter(nome_motorista_id=motorista_id).order_by('timestamp')
                ...
                for viagem in viagens_motorista:
                    ...
                    if data_viagem in timestamps_anteriores:
                        ...
                        # Ignora pausas muito longas (almoço, paradas prolongadas)
                        if diferenca_segundos <= 300:
                            jornadas_por_dia[data_viagem]['tempo_total_segundos'] += diferenca_segundos
                        elif diferenca_segundos > 300 and diferenca_segundos <= 3600:
                            # Registra como nova sessão (pausa entre 5min e 1h)
                            jornadas_por_dia[data_viagem]['sessoes'].append({
                                'inicio': datetime.fromtimestamp(timestamp_anterior),
                                'fim': datetime.fromtimestamp(timestamp_atual),
                                'pausa_minutos': round(diferenca_segundos / 60, 1)
                            })
                ...
                for data_trab, dados in sorted(jornadas_por_dia.items()):
                    ...
                    if dados['timestamps']:
                        primeiro_ts = min(dados['timestamps'])
                        ultimo_ts = max(dados['timestamps'])
                        hora_inicio = datetime.fromtimestamp(primeiro_ts).hour
                        hora_fim = datetime.fromtimestamp(ultimo_ts).hour
                        ...
                        jornada_inicio = datetime.fromtimestamp(primeiro_ts)
                        jornada_fim = datetime.fromtimestamp(ultimo_ts)
                    else:
                        turno = "N/A"
                        jornada_inicio = None
                        jornada_fim = None
                    ...
                    jornadas_detalhadas.append({
                        ...
                        'sessoes_count': len(dados['sessoes']),
