...
def performance_motoristas(request):
    """View para análise de performance de RPM dos motoristas - OTIMIZADA COM PAGINAÇÃO"""
    from datetime import datetime, timedelta
    from django.db.models import Count, Avg, Q, Max, Min, DateField
    ...
    # 🔥 FIX: Não fazer slice na query antes de order_by
    # Ordenar ANTES de qualquer operação
    viagens_eco_query = viagens_eco_query.order_by('-timestamp')
    
    # 🔥 OTIMIZAÇÃO: Limitar registros apenas se for necessário (após ordenação)
    total_registros = viagens_eco_query.count()
    if total_registros > 100000:
        # Criar nova query limitada (sem reutilizar a anterior)
        viagens_eco_limited = viagens_eco_query[:100000]
    else:
        viagens_eco_limited = viagens_eco_query
    
    if viagens_eco_limited.exists():
        # 🔥 OTIMIZAÇÃO: Agrupar por motorista usando SQL aggregation (rápido)
        motoristas_stats = viagens_eco_limited.values('nome_motorista_id').annotate(
            total_pontos=Count('id'),
        ...
        # 🔥 OTIMIZAÇÃO: Buscar todos os motoristas de uma vez (1 query em vez de N)
        motoristas_ids = [s['nome_motorista_id'] for s in motoristas_stats]
    ...
    # 🔥 PAGINAÇÃO: Máximo 10 motoristas por página
    page_number = request.GET.get('page', 1)
    paginator = Paginator(motoristas_performance, 10)  # 10 por página
    ...
    # Calcular estatísticas gerais (usar TODOS os motoristas, não só da página)
    ...
    # 🔥 OTIMIZADO: Preparar dados temporais sem loop manual
    from django.db.models.functions import TruncDate
    
    evolucao_temporal = []
    
    # 🔥 LIMITAR evolução temporal apenas se tiver filtro de período pequeno
    gerar_evolucao = False
    if periodo_dias and periodo_dias != 'todos':
        try:
            dias = int(periodo_dias)
            if dias <= 90:  # Só gerar evolução temporal para períodos <= 90 dias
                gerar_evolucao = True
        except ValueError:
            pass
    
    if gerar_evolucao and viagens_eco_query.exists():
        # 🔥 OTIMIZAÇÃO: Usar agregação SQL em vez de loop Python
        # Limitar a 10.000 registros mais recentes para evolução temporal
        viagens_sample = viagens_eco_query.order_by('-timestamp')[:10000]
        
        evolucao_stats = viagens_sample.annotate(
            data=TruncDate('timestamp', output_field=DateField())
        ...
        for stats in evolucao_stats:
            total = stats['total_pontos']
            if total > 0:
                perc_azul = (stats['faixa_azul_count'] / total * 100)
                perc_verde = (stats['faixa_verde_count'] / total * 100)
                perc_amarela = (stats['faixa_amarela_count'] / total * 100)
                perc_vermelha = (stats['faixa_vermelha_count'] / total * 100)
                perc_ocioso = (stats['motor_ocioso_count'] / total * 100)
                
                score = (perc_verde * 1.0) + (perc_azul * 0.8) - (perc_amarela * 0.5) - (perc_vermelha * 1.0) - (perc_ocioso * 0.3)
                
                evolucao_temporal.append({
                    'data': stats['data'].strftime('%Y-%m-%d'),
                    'score': round(score, 2),
                    'rpm_medio': round((stats['rpm_medio'] / 8) if stats['rpm_medio'] else 0, 1),
                    'velocidade_media': round(stats['velocidade_media'] if stats['velocidade_media'] else 0, 1),
    ...
    context = {
        'empresa_logada': empresa_logada,
        'motoristas_performance': motoristas_paginados,  # 🔥 Usar versão paginada
        'paginator': paginator,  # 🔥 Adicionar paginator
        'page_obj': motoristas_paginados,  # 🔥 Objeto de página
        'stats_gerais': stats_gerais,
...
def jornada_motoristas(request):
    """View simplificada para análise de jornada de trabalho dos motoristas"""
    ...
    # Obter filtros de data
    periodo_dias = request.GET.get('periodo_dias', '7')
    ...
    # Aplicar filtro de período (últimos N dias)
    if periodo_dias and periodo_dias != 'todos':
        try:
            dias = int(periodo_dias)
            data_limite = datetime.now() - timedelta(days=dias)
    ...
    for motorista_id in motoristas_ids:
        try:
            ...
            # Buscar viagens deste motorista
            viagens_motorista = viagens_eco_query.filter(
                nome_motorista_id=motorista_id
            ).values('timestamp', 'unidade_id').order_by('timestamp')[:5000]
            ...
            for data_trab, dados in sorted(jornadas_por_dia.items()):
                # Calcular horas como diferença entre primeiro e último registro
                if dados['primeiro_ts'] and dados['ultimo_ts']:
                    tempo_segundos = dados['ultimo_ts'] - dados['primeiro_ts']
                    ...
                    jornadas_detalhadas.append({
                        ...
                        'hora_inicio': datetime.fromtimestamp(dados['primeiro_ts']).strftime('%H:%M'),
                        'hora_fim': datetime.fromtimestamp(dados['ultimo_ts']).strftime('%H:%M'),
                    })
            ...
            jornadas_motoristas.append({
                'motorista': motorista,
                'motorista_nome': motorista.nm if hasattr(motorista, 'nm') and motorista.nm else f"Motorista #{motorista.id}",
    ...
    # Ordenar por total de horas trabalhadas
    ...
    if jornadas_motoristas:
        stats_gerais = {
            ...
            'total_horas': round(sum(m['total_horas_trabalhadas'] for m in jornadas_motoristas), 2),
            'media_horas': round(sum(m['total_horas_trabalhadas'] for m in jornadas_motoristas) / len(jornadas_motoristas), 2),
            'jornada_completa': sum(1 for m in jornadas_motoristas if m['classificacao'] == "Jornada Completa"),
            'jornada_parcial': sum(1 for m in jornadas_motoristas if m['classificacao'] == "Jornada Parcial"),
            'jornada_reduzida': sum(1 for m in jornadas_motoristas if m['classificacao'] == "Jornada Reduzida"),
        }
    else:
        stats_gerais = {
            'total_motoristas': 0,
