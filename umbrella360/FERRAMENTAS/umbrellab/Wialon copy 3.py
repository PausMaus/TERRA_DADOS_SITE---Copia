...
def Colheitadeira_JSON_03_EX(sid, flag, unit_id, tempo_dias, periodo, reportResourceId, reportTemplateId, reportObjectId, reportObjectSecId):
    ...
    try:
        ...
        comm(f"✅ DataFrame criado com sucesso:")
        comm(f"- Total de registros: {len(relatorio_df)}")
        comm(f"- Linhas principais: {len(relatorio_df[relatorio_df['row_type'] == 'main'])}")
        comm(f"- Sub-linhas: {len(relatorio_df[relatorio_df['row_type'] == 'sub'])}")
        comm(f"- Colunas disponíveis: {relatorio_df.columns.tolist()}")
