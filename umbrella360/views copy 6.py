...
def viagem_diaria(request):
    """View para planilha de viagem diária - navegação entre dias disponíveis, com somatório por motorista E veículo"""
    from datetime import datetime, timedelta
    ...
    # Agrupar por motorista E veículo (chave: unidade + veiculo)
    viagens_agrupadas = viagens_do_dia.values(
        'unidade',  # ID da unidade (motorista)
        'unidade__nm',  # Nome do motorista
        'veiculo'  # ID do veículo - AGORA FAZ PARTE DO AGRUPAMENTO
    ).annotate(
        total_quilometragem=Sum('quilometragem'),
        total_consumido=Sum('Consumido'),
        media_velocidade=Avg('Velocidade_média'),
    ...
    ).order_by('unidade__nm', 'veiculo')  # Ordenar por motorista, depois por veículo
    
    # Calcular estatísticas do dia com base nas somas agrupadas
    stats = {
        'total_unidades': viagens_agrupadas.values('unidade').distinct().count(),  # Número único de motoristas
        'total_veiculos': viagens_agrupadas.values('veiculo').distinct().count(),  # Número único de veículos
        'total_registros': viagens_agrupadas.count(),  # Total de combinações motorista+veículo
    ...
    context = {
        'viagens': viagens_agrupadas,  # Agora agrupado por motorista + veículo
