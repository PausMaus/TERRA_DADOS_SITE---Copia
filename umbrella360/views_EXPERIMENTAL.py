...
def detalhes_unidade(request, unidade_id):
    ...
    # Buscar viagens da unidade
    viagens_unidade = Viagem_Base.objects.filter(unidade_id=unidade.id)

    # Estatísticas gerais
    stats_gerais = viagens_unidade.aggregate(
        total_viagens=Count('id'),
    ...
    stats_por_periodo = viagens_unidade.values('período').annotate(
        quilometragem_periodo=Sum('quilometragem'),
        consumo_periodo=Sum('Consumido'),
        eficiencia_periodo=Avg('Quilometragem_média')
    ).order_by('período')

    # Média do sistema para comparação
    if unidade.cls == 'Motorista':
        media_sistema = Viagem_MOT.objects.aggregate(
            media=Avg('Quilometragem_média')
        )['media'] or 0
    else:
        media_sistema = Viagem_CAM.objects.aggregate(
            media=Avg('Quilometragem_média')
        )['media'] or 0

    # Custo total estimado (considerando R$ 6,00 por litro)
    custo_total = (stats_gerais['total_consumo'] or 0) * 6.0

    # Odômetro para veículos
    odometro_unidade = 0
    if unidade.cls == 'Veículo':
        veiculo = Veiculo.objects.filter(id=unidade.id).first()
        if veiculo and veiculo.odômetro:
            odometro_unidade = veiculo.odômetro
    ...
    # 🔥 OBTER PARÂMETRO DE DATA SELECIONADA DA URL
    data_selecionada_param = request.GET.get('data_timeline')
    
    # Buscar todas as viagens eco para obter datas disponíveis
    viagens_eco_all = Viagem_eco.objects.filter(
        unidade_id=unidade.id
    ).order_by('-timestamp')
    
    # 🔥 OBTER TODAS AS DATAS DISPONÍVEIS
    datas_disponiveis = set()
    for viagem in viagens_eco_all:
        try:
            dt = datetime.fromtimestamp(viagem.timestamp)
            datas_disponiveis.add(dt.date())
        except (ValueError, OSError):
            continue
    
    datas_disponiveis = sorted(list(datas_disponiveis), reverse=True)  # Mais recente primeiro
    
    # 🔥 DETERMINAR DATA A SER EXIBIDA
    data_selecionada = None
    if data_selecionada_param:
        try:
            data_selecionada = datetime.strptime(data_selecionada_param, '%Y-%m-%d').date()
            # Validar se a data existe nos dados
            if data_selecionada not in datas_disponiveis:
                data_selecionada = datas_disponiveis[0] if datas_disponiveis else None
        except ValueError:
            data_selecionada = datas_disponiveis[0] if datas_disponiveis else None
    else:
        # Se não houver parâmetro, usar a data mais recente
        data_selecionada = datas_disponiveis[0] if datas_disponiveis else None
    
    # 🔥 FILTRAR VIAGENS ECO PELO DIA SELECIONADO
    timeline_data = []
    if data_selecionada and viagens_eco_all.exists():
        # Calcular timestamps do dia selecionado (00:00:00 até 23:59:59)
        ...
        viagens_eco_dia = viagens_eco_all.filter(
            timestamp__gte=timestamp_inicio,
            timestamp__lte=timestamp_fim
        ).order_by('timestamp')
        
        # Processar dados para o gráfico
        for viagem_eco in viagens_eco_dia:
            try:
                timestamp_int = int(viagem_eco.timestamp)
                dt = datetime.fromtimestamp(timestamp_int)

                # Validar RPM (dividir por 8 e limitar a 3500)
                if viagem_eco.rpm and float(viagem_eco.rpm/8) <= 3500:
    ...
    # 🔥 CALCULAR ESTATÍSTICAS DO TIMELINE
    timeline_stats = {
        'total_pontos': len(timeline_data),
        'rpm_medio': sum(d['rpm'] for d in timeline_data) / len(timeline_data) if timeline_data else 0,
        'rpm_maximo': max((d['rpm'] for d in timeline_data), default=0),
        'rpm_minimo': min((d['rpm'] for d in timeline_data), default=0),
    ...
    checkpoints_unidade = CheckPoint.objects.filter(unidade_id=unidade.id)
    
    # Total de checkpoints
    total_checkpoints_unidade = checkpoints_unidade.count()
    
    # Cercas mais utilizadas
    cercas_unidade = checkpoints_unidade.values('cerca').annotate(
        total_passagens=Count('id')
    ).order_by('-total_passagens')[:10]
    
    # Número de cercas únicas
    cercas_utilizadas_unidade = checkpoints_unidade.values('cerca').distinct().count()
    
    # Duração média dos checkpoints (em minutos)
    duracao_media_checkpoints = 0
    checkpoints_com_duracao = checkpoints_unidade.exclude(
    ...
    if checkpoints_com_duracao.exists():
        ...
        for cp in checkpoints_com_duracao:
            if cp.data_entrada and cp.data_saida:
                duracao_segundos = (cp.data_saida - cp.data_entrada).total_seconds()
                duracoes.append(duracao_segundos / 60)  # Converter para minutos
        
        if duracoes:
            duracao_media_checkpoints = sum(duracoes) / len(duracoes)
    
    # Checkpoints recentes
    checkpoints_recentes_unidade = checkpoints_unidade.order_by('-data_entrada')[:10]
    
    # Adicionar duração formatada aos checkpoints recentes
    for checkpoint in checkpoints_recentes_unidade:
        if checkpoint.data_entrada and checkpoint.data_saida:
            duracao_segundos = (checkpoint.data_saida - checkpoint.data_entrada).total_seconds()
            horas = int(duracao_segundos // 3600)
            minutos = int((duracao_segundos % 3600) // 60)
            checkpoint.duracao = f"{horas}h {minutos}min" if horas > 0 else f"{minutos}min"
        else:
            checkpoint.duracao = None
    
    # Histórico detalhado (limitado a 20)
    checkpoints_detalhados_unidade = checkpoints_unidade.order_by('-data_entrada')[:20]
    
    # Adicionar duração aos checkpoints detalhados
    ...
    infracoes_unidade = Infrações.objects.filter(unidade_id=unidade.id)
    
    # Total de infrações
    total_infracoes_unidade = infracoes_unidade.count()
    
    # Estatísticas de velocidade
    velocidade_media_infracoes_unidade = infracoes_unidade.aggregate(
        media=Avg('velocidade')
    )['media'] or 0
    
    velocidade_maxima_infracoes_unidade = infracoes_unidade.aggregate(
        maxima=Max('velocidade')
    )['maxima'] or 0
    
    limite_medio_infracoes_unidade = infracoes_unidade.aggregate(
        media=Avg('limite')
    )['media'] or 0
    
    # Limites mais infringidos
    limites_unidade = infracoes_unidade.values('limite').annotate(
        total_ocorrencias=Count('id')
    ).order_by('-total_ocorrencias')[:10]
    
    # Infrações recentes
    infracoes_recentes_unidade = infracoes_unidade.order_by('-data')[:10]
    
    # Histórico detalhado de infrações (limitado a 20)
    infracoes_detalhadas_unidade = infracoes_unidade.order_by('-data')[:20]
    ...
    context = {
        ...
        'duracao_media_checkpoints': round(duracao_media_checkpoints, 1),
        ...
        'velocidade_media_infracoes_unidade': velocidade_media_infracoes_unidade,
        'velocidade_maxima_infracoes_unidade': velocidade_maxima_infracoes_unidade,
        'limite_medio_infracoes_unidade': limite_medio_infracoes_unidade,
...
def performance_frota(request):
    """View para análise de performance de RPM da frota toda"""
    # Verificar se há empresa logada
    empresa_logada_id = request.session.get('empresa_logada')
    empresa_logada = None
    ...
    # Obter parâmetro de data selecionada
    data_selecionada_param = request.GET.get('data_performance')
    periodo_selecionado = request.GET.get('periodo', 'Últimos 30 dias')
    
    # Buscar todas as viagens eco da empresa
    from .models import Viagem_eco
    ...
    # Obter todas as datas disponíveis
    datas_disponiveis = set()
    for viagem in viagens_eco_query.order_by('-timestamp'):
        try:
            dt = datetime.fromtimestamp(viagem.timestamp)
            datas_disponiveis.add(dt.date())
        except (ValueError, OSError):
    ...
    if data_selecionada and viagens_eco_query.exists():
        # Calcular timestamps do dia selecionado
        inicio_dia = datetime.combine(data_selecionada, datetime.min.time())
        fim_dia = datetime.combine(data_selecionada, datetime.max.time())
        ...
        # Filtrar viagens do dia
        viagens_eco = viagens_eco_query.filter(
            timestamp__gte=timestamp_inicio,
            timestamp__lte=timestamp_fim
        )
        
        # Agrupar por unidade e calcular faixas de RPM
        unidades_ids = viagens_eco.values_list('unidade_id', flat=True).distinct()
        
        for unidade_id in unidades_ids:
            try:
                unidade = Unidade.objects.select_related('empresa').get(id=unidade_id)
                viagens_unidade = viagens_eco.filter(unidade_id=unidade_id)
                
                # Calcular dados de RPM
                rpms = []
