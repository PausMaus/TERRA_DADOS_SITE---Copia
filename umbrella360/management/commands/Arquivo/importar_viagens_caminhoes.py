...
from umbrella360.models import Viagem_CAM, Caminhao
import os

class Command(BaseCommand):
    help = 'Importa viagens únicas dos caminhões de um arquivo Excel (.xlsx) - evita duplicatas'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            # Buscar todos os caminhões existentes
            caminhoes_dict = {
                caminhao.agrupamento: caminhao 
                for caminhao in Caminhao.objects.all()
            }

            # Verificar caminhões que não existem no banco
            agrupamentos_arquivo = df['Agrupamento'].str.strip().unique()
            caminhoes_nao_encontrados = []
            
            for agrupamento in agrupamentos_arquivo:
                if agrupamento not in caminhoes_dict:
                    caminhoes_nao_encontrados.append(agrupamento)

            if caminhoes_nao_encontrados:
                self.stderr.write(f'❌ Caminhões não encontrados no banco de dados:')
                for caminhao in caminhoes_nao_encontrados[:10]:  # Mostra apenas os primeiros 10
                    self.stderr.write(f'  - {caminhao}')
                if len(caminhoes_nao_encontrados) > 10:
                    self.stderr.write(f'  ... e mais {len(caminhoes_nao_encontrados) - 10}')
                self.stderr.write(f'Total de caminhões não encontrados: {len(caminhoes_nao_encontrados)}')
                self.stderr.write('Execute primeiro o comando IMP_CAM para importar os caminhões.')
                return
            ...
            for _, linha in df.iterrows():
                ...
                # Buscar caminhão
                caminhao_obj = caminhoes_dict.get(agrupamento_nome)
                if not caminhao_obj:
                    viagens_com_erro.append(f'{agrupamento_nome} (não encontrado)')
                    continue

                # Verificar se já existe uma viagem para este caminhão e mês
                viagem_existente = Viagem_CAM.objects.filter(
                    agrupamento=caminhao_obj,
                    mês=mes
                ).first()
            ...
            novos_criados = 0
            if viagens_para_criar:
                Viagem_CAM.objects.bulk_create(viagens_para_criar)
            ...
            total_banco = Viagem_CAM.objects.count()
