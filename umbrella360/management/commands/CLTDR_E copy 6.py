...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Opção 1: Processar últimos 30 dias de todas as empresas
        #self.gerar_estatisticas_de_viagens_eco(dias=30)
        #self.gerar_estatisticas_de_viagens_eco_02(dias=30)
        
        # Opção 2: Processar últimos 7 dias da empresa Petitto
        #self.gerar_estatisticas_de_viagens_eco(empresa="Petitto", dias=7)
        
        # Opção 3: Processar TODOS os dados históricos (pode demorar)
        #self.gerar_estatisticas_de_viagens_eco()

        self.adicionar_dados_combustivel_estatisticas()
...
########################################################################################
    ...
    def MENSAGENS(self, tempo, empresa):
        def comm(msg):
            print(colored("="*30, "yellow"))
            print(colored(f"MENSAGENS:","green"))
            print(f"{msg}")
            print(colored("="*30, "yellow"))
    ...
    def gerar_estatisticas_de_viagens_eco(self, empresa=None, dias=None):
        """
        🔥 FUNÇÃO STANDALONE: Processa dados de Viagem_eco E Viagem_Detalhada para criar Estatistica_Diaria_Motorista
        ...
        print(f"\n{'='*70}")
        print(f"🔥 GERANDO ESTATÍSTICAS DIÁRIAS (VIAGEM_ECO + VIAGEM_DETALHADA)")
        print(f"{'='*70}")
    ...
    def gerar_estatisticas_de_viagens_eco_02(self, empresa=None, dias=None):
        """
        🔥 FUNÇÃO STANDALONE: Processa dados de Viagem_eco E Viagem_Detalhada para criar Estatistica_Diaria_Motorista
        Inclui estatísticas de RPM/faixas E dados de consumo de combustível
    ...
    def adicionar_dados_combustivel_estatisticas(self, empresa=None, dias=None):
        """
        🔥 Adiciona dados de combustível das Viagens_Detalhadas às estatísticas existentes
        Complementa os dados de Estatistica_Diaria_Motorista com informações de consumo
        ...
        print(f"\n{'='*70}")
        print(f"⛽ ADICIONANDO DADOS DE COMBUSTÍVEL ÀS ESTATÍSTICAS")
        print(f"{'='*70}")
        
        # Construir query para Viagem_Detalhada
        viagens_det_query = Viagem_Detalhada.objects.select_related('unidade__empresa')
        
        # Aplicar filtros
        if empresa:
            viagens_det_query = viagens_det_query.filter(unidade__empresa__nome=empresa)
            print(f"📊 Empresa filtrada: {colored(empresa, 'cyan')}")
        else:
            print(f"📊 Processando: {colored('TODAS as empresas', 'cyan')}")
        ...
        if dias:
            data_limite = datetime.now() - timedelta(days=dias)
            timestamp_limite = int(data_limite.timestamp())
            viagens_det_query = viagens_det_query.filter(timestamp_inicial__gte=timestamp_limite)
            print(f"📅 Período: {colored(f'Últimos {dias} dias', 'cyan')}")
        else:
            print(f"📅 Período: {colored('TODOS os dados históricos', 'yellow')}")
        
        # Filtrar viagens com dados válidos
        viagens_det_query = viagens_det_query.exclude(Quilometragem_média__isnull=True)
        viagens_det_query = viagens_det_query.exclude(Consumido__isnull=True)
        
        total_viagens_det = viagens_det_query.count()
        print(f"🚛 Total de viagens detalhadas: {colored(f'{total_viagens_det:,}', 'green')}")
        
        if total_viagens_det == 0:
            print(f"⚠️ {colored('Nenhuma viagem detalhada encontrada!', 'yellow')}")
            return
        
        print(f"\n{'─'*70}")
        print(f"🔄 Agrupando dados de combustível por unidade/data...")
        print(f"{'─'*70}\n")
        
        # Agrupar viagens detalhadas: {(unidade_id, data): {quilometragem, consumo}}
        viagens_por_unidade_dia = defaultdict(lambda: {
        ...
        for viagem_det in viagens_det_query.iterator(chunk_size=batch_size):
            try:
                data_viagem = date.fromtimestamp(viagem_det.timestamp_inicial)
                unidade_id = viagem_det.unidade.id
                
                key = (unidade_id, data_viagem)
                viagens_por_unidade_dia[key]['quilometragem'] += Decimal(str(viagem_det.quilometragem or 0))
                viagens_por_unidade_dia[key]['consumo'] += Decimal(str(viagem_det.Consumido or 0))
                viagens_por_unidade_dia[key]['count'] += 1
                
                total_processadas += 1
                
                if total_processadas % 10000 == 0:
                    print(f"   ⚙️ Processadas: {colored(f'{total_processadas:,}', 'cyan')} / {colored(f'{total_viagens_det:,}', 'green')} ({total_processadas/total_viagens_det*100:.1f}%)")
            
            except Exception as e:
                print(f"   ⚠️ Erro ao processar viagem: {e}")
                continue
        
        print(f"\n✅ Viagens processadas: {colored(f'{total_processadas:,}', 'green')}")
        print(f"📊 Combinações (unidade, data): {colored(len(viagens_por_unidade_dia), 'cyan')}")
        
        # 🔥 NOVA ESTRATÉGIA: Vincular motoristas diretamente através de Viagem_Detalhada
        print(f"\n{'─'*70}")
        print(f"🔗 Vinculando dados de combustível aos motoristas...")
        print(f"{'─'*70}\n")
        
        # Mapear motoristas por (unidade, data) através de Viagem_eco
        print(f"📍 Etapa 1: Mapeando motoristas que usaram cada veículo por dia...")
        
        # Query Viagem_eco para mapear qual motorista usou qual veículo em qual dia
        viagens_eco_query = Viagem_eco.objects.filter(nome_motorista__isnull=False)
        
        if empresa:
            viagens_eco_query = viagens_eco_query.filter(unidade__empresa__nome=empresa)
        
        if dias:
            data_limite_dt = datetime.now() - timedelta(days=dias)
            timestamp_limite = int(data_limite_dt.timestamp())
            viagens_eco_query = viagens_eco_query.filter(timestamp__gte=timestamp_limite)
        
        # Mapear: {(unidade_id, data): motorista_id}
        from collections import defaultdict
        veiculo_motorista_dia = defaultdict(set)  # Pode ter múltiplos motoristas por veículo/dia
        
        print(f"⏳ Processando Viagem_eco para mapear motoristas...")
        total_eco_processadas = 0
        
        for viagem_eco in viagens_eco_query.values('unidade_id', 'timestamp', 'nome_motorista_id').iterator(chunk_size=10000):
            try:
                data_viagem = date.fromtimestamp(viagem_eco['timestamp'])
                unidade_id = viagem_eco['unidade_id']
                motorista_id = viagem_eco['nome_motorista_id']
                
                key = (unidade_id, data_viagem)
                veiculo_motorista_dia[key].add(motorista_id)
                
                total_eco_processadas += 1
                
                if total_eco_processadas % 50000 == 0:
                    print(f"   ⚙️ Processadas: {colored(f'{total_eco_processadas:,}', 'cyan')} viagens eco...")
            
            except Exception as e:
                continue
        
        print(f"✅ Mapeamento completo: {colored(f'{len(veiculo_motorista_dia):,}', 'green')} combinações (veículo, data)")
        
        # Agora vincular dados de combustível aos motoristas
        print(f"\n📍 Etapa 2: Atualizando estatísticas de motoristas com dados de combustível...")
        
        estatisticas_atualizadas = 0
        erros = 0
        sem_dados_combustivel = 0
        vinculacoes_encontradas = 0
        
        # Buscar estatísticas existentes
        stats_query = Estatistica_Diaria_Motorista.objects.select_related('motorista')
        
        if empresa:
            stats_query = stats_query.filter(motorista__empresa__nome=empresa)
        
        if dias:
            data_limite_date = (datetime.now() - timedelta(days=dias)).date()
            stats_query = stats_query.filter(data__gte=data_limite_date)
        
        total_stats = stats_query.count()
        print(f"📊 Total de estatísticas a atualizar: {colored(f'{total_stats:,}', 'cyan')}")
        
        for estatistica in stats_query.iterator(chunk_size=1000):
            try:
                # Buscar quais veículos o motorista usou neste dia
                # Procurar nos dados mapeados
                quilometragem_total = Decimal('0.00')
                consumo_total = Decimal('0.00')
                viagens_count = 0
                
                # Percorrer todos os veículos e verificar se este motorista os usou
                for (unidade_id, data_veiculo), motoristas_set in veiculo_motorista_dia.items():
                    # Se a data bate e o motorista está no set
                    if data_veiculo == estatistica.data and estatistica.motorista.id in motoristas_set:
                        # Buscar dados de combustível deste veículo neste dia
                        key_combustivel = (unidade_id, data_veiculo)
                        if key_combustivel in viagens_por_unidade_dia:
                            dados = viagens_por_unidade_dia[key_combustivel]
                            quilometragem_total += dados['quilometragem']
                            consumo_total += dados['consumo']
                            viagens_count += dados['count']
                            vinculacoes_encontradas += 1
                
                # Atualizar estatística se houver dados de combustível
                if viagens_count > 0:
                    km_media = quilometragem_total / consumo_total if consumo_total > 0 else Decimal('0.00')
                    
                    estatistica.quilometragem_total = quilometragem_total
                    estatistica.consumo_total = consumo_total
                    estatistica.Quilometragem_média = km_media
                    estatistica.total_viagens_detalhadas = viagens_count
                    estatistica.save()
                    
                    estatisticas_atualizadas += 1
                    
                    if estatisticas_atualizadas % 100 == 0:
                        print(f"   💾 Atualizadas: {colored(estatisticas_atualizadas, 'cyan')} estatísticas...")
                else:
                    sem_dados_combustivel += 1
            
            except Exception as e:
                erros += 1
                print(f"   ❌ Erro ao atualizar estatística {estatistica.id}: {e}")
                continue
        ...
        print(f"📊 RESUMO FINAL")
        print(f"{'='*70}")
        print(f"🔗 Vinculações encontradas (motorista→veículo→combustível): {colored(vinculacoes_encontradas, 'cyan')}")
        print(f"✅ Estatísticas ATUALIZADAS com dados de combustível: {colored(estatisticas_atualizadas, 'green')}")
        print(f"⚠️ Estatísticas SEM dados de combustível: {colored(sem_dados_combustivel, 'yellow')}")
        print(f"❌ Erros: {colored(erros, 'red')}")
        if total_stats > 0:
            print(f"📈 Taxa de sucesso: {colored(f'{(estatisticas_atualizadas / total_stats * 100):.1f}%', 'green')}")
        print(f"\n💡 DEBUG INFO:")
        print(f"   - Total de veículos/dias mapeados: {colored(len(veiculo_motorista_dia), 'cyan')}")
        print(f"   - Total de veículos/dias com combustível: {colored(len(viagens_por_unidade_dia), 'cyan')}")
        print(f"{'='*70}\n")
